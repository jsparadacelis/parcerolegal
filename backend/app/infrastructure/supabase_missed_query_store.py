"""Supabase adapter para MissedQueryStore — INSERT vía REST (PostgREST).
...
propaga — persistir un vacío de corpus jamás debe romper la respuesta al usuario.
...
class SupabaseMissedQueryStore:
    def __init__(self, url: str, api_key: str, table: str = "missed_queries") -> None:
    ...
    def save(self, missed: MissedQuery) -> None:
        try:
            response = requests.post(
                self._insert_url,
                json=dataclasses.asdict(missed),
            ...
            logger.warning(
                "no se pudo guardar la pregunta fuera de alcance en Supabase",
                exc_info=True,
            )
